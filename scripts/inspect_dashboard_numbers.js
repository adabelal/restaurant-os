...
async function inspect() {
    ...
    if (firstBank) console.log(`   Date Début: ${firstBank.date.toISOString().split('T')[0]}`)
    if (lastBank) console.log(`   Date Fin:   ${lastBank.date.toISOString().split('T')[0]}`)
    ...
    console.log(`   Entrées (Ventes): +${cashIn._sum.amount ? cashIn._sum.amount.toFixed(2) : 0} €`)
    console.log(`   Sorties (Dépôts?): ${cashOut._sum.amount ? cashOut._sum.amount.toFixed(2) : 0} €`)
