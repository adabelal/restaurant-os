        ...
        <main className="flex min-h-screen flex-col bg-background transition-colors duration-300">
            ...
            <div className="flex-1 px-8 py-8 md:px-12 max-w-7xl mx-auto w-full">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    <div className="bg-card p-6 rounded-2xl border shadow-sm flex flex-col gap-2 transition-all hover:shadow-md">
                        <span className="text-sm font-medium text-muted-foreground flex items-center gap-2">
                            <ArrowUpCircle className="h-4 w-4 text-emerald-500" /> Total Entrées
                        </span>
                        <span className="text-2xl font-bold text-emerald-600">+{totalIn.toLocaleString('fr-FR', { style: 'currency', currency: 'EUR' })}</span>
                    </div>
                    <div className="bg-card p-6 rounded-2xl border shadow-sm flex flex-col gap-2 transition-all hover:shadow-md">
                        <span className="text-sm font-medium text-muted-foreground flex items-center gap-2">
                            <ArrowDownCircle className="h-4 w-4 text-rose-500" /> Total Sorties
                        </span>
                        <span className="text-2xl font-bold text-rose-600">-{totalOut.toLocaleString('fr-FR', { style: 'currency', currency: 'EUR' })}</span>
                    </div>
                    <div className="bg-card p-6 rounded-2xl border shadow-sm flex flex-col gap-2 ring-2 ring-blue-500/20 transition-all hover:shadow-md">
                        <span className="text-sm font-medium text-muted-foreground flex items-center gap-2">
                            <Calculator className="h-4 w-4 text-blue-500" /> Solde Théorique
                        </span>
                        <span className={`text-2xl font-bold ${balance >= 0 ? 'text-blue-600' : 'text-rose-600'}`}>
                ...
                <Tabs defaultValue="entrees" className="w-full space-y-8">
                    <div className="flex justify-center md:justify-start">
                        <TabsList className="p-1 bg-muted rounded-2xl w-auto inline-flex shadow-inner border border-border/50">
                            <TabsTrigger value="entrees" className="px-8 py-2.5 gap-2 rounded-xl data-[state=active]:bg-background data-[state=active]:shadow-md transition-all font-bold">
                                <ArrowUpCircle className="h-4 w-4 text-emerald-500" /> Entrées
                            </TabsTrigger>
                            <TabsTrigger value="sorties" className="px-8 py-2.5 gap-2 rounded-xl data-[state=active]:bg-background data-[state=active]:shadow-md transition-all font-bold">
                                <ArrowDownCircle className="h-4 w-4 text-rose-500" /> Sorties
                            </TabsTrigger>
                            <TabsTrigger value="stats" className="px-8 py-2.5 gap-2 rounded-xl data-[state=active]:bg-background data-[state=active]:shadow-md transition-all font-bold">
                                <TrendingUp className="h-4 w-4 text-blue-500" /> Analyses
                            </TabsTrigger>
                            <TabsTrigger value="categories" className="px-8 py-2.5 gap-2 rounded-xl data-[state=active]:bg-background data-[state=active]:shadow-md transition-all font-bold">
